...
Ray Camera::generate_ray(double x, double y) const {
  ...
  Vector3D corner_leftbottom(-tan(radians(hFov)*.5), -tan(radians(vFov)*.5), -1); //[0 0]
  Vector3D corner_rightup(tan(radians(hFov)*.5), tan(radians(vFov)*.5), -1); //[1 1]

  double xlen = corner_rightup.x - corner_leftbottom.x;
  double ylen = corner_rightup.y - corner_leftbottom.y;

  Vector3D dir(corner_leftbottom.x + x * xlen,
	       corner_leftbottom.y + y * ylen,
	       -1);
  dir = c2w * dir;
  dir.normalize();
  return Ray(pos, dir);
