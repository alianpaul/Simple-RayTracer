...
Spectrum PathTracer::estimate_direct_lighting(const Ray& r, const Intersection& isect) {
  ...
  for(SceneLight* light : scene->lights)
    {
      //1. do ns_area_light sample from each light;
      size_t ns = (light->is_delta_light()) ? 1 : ns_area_light;
      Spectrum L_out_light;
      
      //2. sum each sample radiance
      for(size_t i = 0; i < ns; ++i)
	{
	  Vector3D w_inw; //world space
	  float    distToLight;
	  float    pdf;
	  Spectrum Ls = light->sample_L(hit_p, &w_inw, &distToLight, &pdf);

	  Vector3D w_in = w2o * w_inw; //object space
	  w_in.normalize();
	  
	  //2.1 test if the direct light comes in the negative direction(bellow the surface),
	  if(w_in.z < 0) continue;
	  
	  //2.2 test if the direct light is obsecured by other objects in the scene
	  Ray direct_ray(hit_p + EPS_D * w_inw, w_inw, distToLight);
	  if(bvh->intersect(direct_ray)) continue;
	  
	  //2.3 calculate the BRDF value at hit_p and accumulate the result
	  Spectrum fr = isect.bsdf->f(w_out, w_in);
	  L_out_light += (fr * Ls * w_in.z) / pdf;
	}
...
Spectrum PathTracer::estimate_indirect_lighting(const Ray& r, const Intersection& isect) {
  ...
  w_in.normalize();
  Vector3D w_inw = o2w * w_in;//in world coordinate
  w_inw.normalize();
  
  float russian_alive = fr.illum() * 20 + 0.05;
  float russian_dead  = clamp((1 - russian_alive), 0.f, 1.f - EPS_F);
  if(!coin_flip(russian_dead)) //The depth is taken care of by trace_ray
    {
      Ray indirect_ray(hit_p + EPS_D*w_inw, w_inw, int(r.depth - 1));
      Spectrum L = trace_ray(indirect_ray, isect.bsdf->is_delta());
      return fr*L*abs_cos_theta(w_in) / (pdf * (1.f - russian_dead));
    }
  //Russian dead
  return Spectrum();
}
...
Spectrum PathTracer::trace_ray(const Ray &r, bool includeLe) {
  ...
  //std::cout << "R " << r.depth << std::endl;
  //std::cout << "S " << L_out << std::endl;  
  return L_out;
}
...
Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
  ...
  Spectrum radiance_sum;

  float s1 = 0; //illum sum
  float s2 = 0; //illum^2 sum
  ...
  for(int n = 1; n <= num_samples; ++n)
	...
	sample += gridSampler->get_sample();
      Ray ray_camera = camera->generate_ray(sample.x/(double)sampleBuffer.w,
					    sample.y/(double)sampleBuffer.h);
      ray_camera.depth = max_ray_depth;
      Spectrum L_s = trace_ray(ray_camera, true);
      
      //update s1 s2
      s1 += L_s.illum();
      s2 += L_s.illum() * L_s.illum();
      radiance_sum += L_s;
	...
	{
	  float mean = s1 / n;
	  float var2 = (1.f / (n - 1) ) * (s2 - s1 * s1 / n);
	  float var  = sqrt(var2);
	  float I = 1.96f * var / sqrt((float)n);
	  if(I <= maxTolerance * mean)
	    {
	      //converged
	      sampleCountBuffer[y * frameBuffer.w + x] = n;
	      return radiance_sum / n;
	    }
	}
    }

  sampleCountBuffer[y * frameBuffer.w + x] = num_samples;
  return radiance_sum / num_samples;
